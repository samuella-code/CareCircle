...
function App() {
  ...
  return <div className="app">
    ...
    {mode === 'senior' ? <Senior onVoice={()=>setVoice(true)} onHelp={()=>setHelp(true)} onCheckin={()=>setCheckin(true)} checked={checked} meds={meds} markTaken={markTaken} notify={notify}/> : <Caregiver meds={meds} tasks={tasks} setTasks={setTasks} checked={checked} mood={mood} alerts={alerts} setAlerts={setAlerts} notify={notify}/>} 
...
function Senior({onVoice,onHelp,onCheckin,checked,meds,markTaken,notify}:{onVoice:()=>void,onHelp:()=>void,onCheckin:()=>void,checked:boolean,meds:Medicine[],markTaken:(id:number)=>void,notify:(m:string)=>void}) {
...
function Caregiver({meds,tasks,setTasks,checked,mood,alerts,setAlerts,notify}:{meds:Medicine[],tasks:CareTask[],setTasks:React.Dispatch<React.SetStateAction<CareTask[]>>,checked:boolean,mood:string,alerts:Alert[],setAlerts:React.Dispatch<React.SetStateAction<Alert[]>>,notify:(m:string)=>void}) { const [taskModal,setTaskModal]=useState(false); const [activeNav,setActiveNav]=useState('overview'); const toggleTask=(id:number)=>setTasks(list=>list.map(t=>t.id===id?{...t,done:!t.done}:t)); const go=(id:string)=>{setActiveNav(id);document.getElementById(id)?.scrollIntoView({behavior:'smooth',block:'center'})}; const navItems=[[Home,'Overview','overview'],[Activity,'Health & activity','activity'],[Pill,'Medications','medications'],[CalendarDays,'Appointments','appointments'],[ClipboardList,'Care tasks','tasks'],[Users,'Family circle','family']] as const; return <main className="dashboard"><aside className="sidebar"><Logo/><div className="profile"><span>AO</span><div><b>Amara Okafor</b><small>Primary caregiver</small></div></div><nav>{navItems.map(([Icon,label,id])=><button onClick={()=>go(id)} className={activeNav===id?'selected':''} key={id}><Icon/>{label}</button>)}</nav><div className="privacy"><ShieldCheck/><b>Privacy protected</b><small>Grace controls what her circle can see.</small></div></aside>
